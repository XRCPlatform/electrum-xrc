...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-btr-macos
	cd /tmp/electrum-btr-macos
...
rm -rf /tmp/electrum-btr-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-btr-macos/image/
cp -r $1 /tmp/electrum-btr-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum-BTR_uncompressed.dmg \
    /tmp/electrum-btr-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum-BTR_uncompressed.dmg electrum-btr-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum-BTR_uncompressed.dmg

echo "Done."
md5sum electrum-btr-$VERSION.dmg
