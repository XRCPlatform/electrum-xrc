...
def codesign(identity, binary):
    ...
    os.system("codesign -v -f -s '{}' '{}'".format(identity, binary)
              ) == 0 or fail("Could not code sign " + binary)
    if saved_dir:
        os.chdir(saved_dir)
...
def monkey_patch_pyinstaller_for_codesigning(identity):
    ...
    def my_func(fn, distname):
        ...
        if (fn, distname) not in _signed:
            ...
            # remember we signed it so we don't sign again
            _signed.add((fn, distname))
...
datas += [(electrum + "contrib/osx/CalinsQRReader/build/Release/CalinsQRReader.app",
           "./contrib/osx/CalinsQRReader/build/Release/CalinsQRReader.app")]

# Add libusb so Trezor and Safe-T mini will work
binaries = [(electrum + "contrib/osx/libusb-1.0.dylib", ".")]
binaries += [(electrum + "contrib/osx/libsecp256k1.0.dylib", ".")]
binaries += [(electrum + "contrib/osx/libx13.dylib", ".")]
...
qt_bins2remove = ('qtweb', 'qt3d', 'qtgame', 'qtdesigner',
                  'qtquick', 'qtlocation', 'qttest', 'qtxml')
