...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-xrc-macos
	cd /tmp/electrum-xrc-macos
...
rm -rf /tmp/electrum-xrc-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-xrc-macos/image/
cp -r $1 /tmp/electrum-xrc-macos/image/
